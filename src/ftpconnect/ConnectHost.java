	...
	private static boolean resolveIPAddress(String ip_address) {
		try {
			...
			JOptionPane.showMessageDialog(ClientMainFrame.frame,
					"Enter a valid host name or check your internet connection.", "Unable to resolve host",
	...
	private static boolean FTPConnect() {
		...
		try {
			if (port.isEmpty())
				...
				ftpClient.connect(ipaddress, Integer.valueOf(port));
			...
			if (!connected) {
				JOptionPane.showMessageDialog(ClientMainFrame.frame, "Invalid credentials. Please try again.",
						"Authentication Failed", JOptionPane.OK_OPTION, new ImageIcon("res\\alert_icon.png"));
			...
			ClientMainFrame.loadDialog.dispose();
			JOptionPane.showMessageDialog(ClientMainFrame.frame, "Check your connection and try again",
					"Connection Timed Out", JOptionPane.OK_OPTION, new ImageIcon("res\\alert_icon.png"));
			ClientMainFrame.resetFields();
