...
public class ButtonActionListener implements ActionListener {
            ...
            protected Void doInBackground() throws Exception {
                try (FileInputStream inputStream = new FileInputStream(new File(uploadSelectedPath))) {
                    if (!RemoteTreeLoad.ftpClient.storeFile(remoteSelected, inputStream)) {
                        JOptionPane.showMessageDialog(ClientMainFrame.frame, "File couldn't be uploaded. Please try again.", "Upload unsuccessful", JOptionPane.OK_OPTION, new ImageIcon("res\\alert_icon.png"));
                    } else {
                        JOptionPane.showMessageDialog(ClientMainFrame.frame, "File uploaded successfully to the server", "Upload Successful", JOptionPane.OK_OPTION, new ImageIcon("res\\check_icon.png"));

                        ClientMainFrame.defaultRemoteListModel.removeAllElements();
                        FTPFile[] files = RemoteTreeLoad.ftpClient.listFiles(createPath(remotePath), new FTPFileFilter() {
                            @Override
                            public boolean accept(FTPFile ftpFile) {
                                return (!ftpFile.isDirectory());
                            }
                        });
                        if (files.length == 0) ClientMainFrame.defaultRemoteListModel.removeAllElements();
                        else {
    ...
    protected void deleteRemoteFolder() {
        if (RemoteTreeLoad.remoteTree.getSelectionPath() != null) {
            ...
            if (selection == JOptionPane.OK_OPTION) {
                ...
                SwingWorker<Void, Void> swingWorker = new SwingWorker<Void, Void>() {
                    ...
                    protected Void doInBackground() throws Exception {
                        if (removeDirectory(RemoteTreeLoad.ftpClient, parentPath, name)) {
                            ...
                            JOptionPane.showMessageDialog(ClientMainFrame.frame, "Couldn't delete the directory. Please try again.", "Operation failed", JOptionPane.OK_OPTION, new ImageIcon("res\\alert_icon.png"));
                        }
                        return null;
                    }
                ...
                JOptionPane.showMessageDialog(ClientMainFrame.frame, "Choose a directory to delete", "Delete directory", JOptionPane.OK_OPTION, new ImageIcon("res\\alert_icon.png"));
            }
        }
    }
        ...
        if (newName != null) {
            ...
            try {
                if (!RemoteTreeLoad.ftpClient.makeDirectory(selectedPath)) {
                    JOptionPane.showMessageDialog(ClientMainFrame.frame, "Couldn't create new directory. Please try again", "Operation failed", JOptionPane.OK_OPTION, new ImageIcon("res\\alert_icon.png"));
                } else {
                    showServerReply(RemoteTreeLoad.ftpClient);
                    DefaultMutableTreeNode selectedNode = (DefaultMutableTreeNode) RemoteTreeLoad.remoteTree.getLastSelectedPathComponent();
                ...
                protected Void doInBackground() throws Exception {
                    ...
                    try (FileOutputStream fileOutputStream = new FileOutputStream(downloadLocalPath)) {
                        if (!RemoteTreeLoad.ftpClient.retrieveFile(selectedPath, fileOutputStream)) {
                            JOptionPane.showMessageDialog(ClientMainFrame.frame, "Download was unsuccessful. Please try again.", "Operation Failed", JOptionPane.OK_OPTION, new ImageIcon("res\\alert_icon.png"));
                            System.out.println("Download failed");
                        } else {
                            JOptionPane.showMessageDialog(ClientMainFrame.frame, "File has been successfully downloaded.", "Download successfull", JOptionPane.OK_OPTION, new ImageIcon("res\\check_icon.png"));
                        }
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
        ...
        if (selection == JOptionPane.OK_OPTION) {
            ...
            try {
                ...
                if (RemoteTreeLoad.ftpClient.deleteFile(selectedPath)) {
                    ...
                    JOptionPane.showMessageDialog(ClientMainFrame.frame, "Operation failed. Try again.", "Operation failed", JOptionPane.OK_OPTION, new ImageIcon("res\\alert_icon.png"));
                }
        ...
        try {
            ...
            if (newName != null) {
                while (newName == null || check.equals("")) {
                    JOptionPane.showMessageDialog(ClientMainFrame.frame, "Empty field not allowed", "Invalid name", JOptionPane.OK_OPTION, new ImageIcon("res\\alert_icon.png"));
                    newName = (String) JOptionPane.showInputDialog(ClientMainFrame.frame, "Enter the new name:", "Rename", JOptionPane.INFORMATION_MESSAGE, new ImageIcon("res\\bluealert.png"), null, null);
                    if (newName != null) check = newName.replaceAll(" ", "");
                }
                TreePath path = RemoteTreeLoad.remoteTree.getSelectionPath();
            ...
            JOptionPane.showMessageDialog(ClientMainFrame.frame, "Connection to server lost. Please try again.", "Connection Lost", JOptionPane.OK_OPTION, new ImageIcon("res\\alert_icon.png"));
            e.printStackTrace();
        }
    ...
    public static String createPath(TreePath node) {
        ...
        for (int i = 0; i < nodes.length; i++) {
            if (nodes[i].toString().equals("/")) continue;
